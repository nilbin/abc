 ...
 *  (url.ts owns the query grammar; navigation clears the page-scoped `record` param). */
export function NavProvider(props: { surface?: string; children: ReactNode }) {
  ...
  useEffect(() => {
    const onPop = () => { const n = readQuery(); setModeId(n.mode); setPageId(n.page); };
...
export function NavPage() {
  ...
  if (node.target.page) {
    ...
    // Keyed by page: page identity resets record state — a record never survives navigation.
    if (manifest.pages?.[node.target.page]) return <ModelPage key={node.target.page} page={node.target.page} />;
    return null;
  }
