        ...
        Input input, OperationContext context, ITamDb tam, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(input.Name))
            return Task.FromResult<Result<Output>>(
                InspectFindings.NameRequired.At(nameof(Input.Name)));
        if (string.IsNullOrWhiteSpace(input.OrderType))
            return Task.FromResult<Result<Output>>(
                InspectFindings.OrderTypeRequired.At(nameof(Input.OrderType)));
...
public static class AddTemplateItem
{
        ...
        Input input, OperationContext context, ITamDb tam, CancellationToken ct)
    {
        ...
        if (template.Retired)
            return InspectFindings.TemplateRetired.At(nameof(Input.TemplateId));
        if (string.IsNullOrWhiteSpace(input.Text))
            return InspectFindings.ItemTextRequired.At(nameof(Input.Text));
...
public static class PassChecklist
{
        ...
        Input input, OperationContext context, ITamDb tam, CancellationToken ct)
    {
        ...
        if (open > 0)
            return InspectFindings.ItemsOpen.With(("open", open));
